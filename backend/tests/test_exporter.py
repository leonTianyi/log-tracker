...
def test_export_header_has_paths_then_amendment_keys():
    ...
    header = _rows(text)[0]
    assert header[:2] == ["ics-1ahs-prod", "ics-cfh-prod"]
...
def test_unset_flags_export_blank_set_flags_render_1_0():
    ...
    flags = (
        b"ics-1ahs-prod,ics-cfh-prod,disable_sa_filter,add_mod2_channels\n"
        b"s3://ics-1ahs-prod/data/TPG/2026.05/collection_a/01.12.32_checkout_run/,,1,0\n"
    )
    ...
    may = by_key["s3://ics-1ahs-prod/data/TPG/2026.05/collection_a/01.12.32_checkout_run/"]
    assert may[di] == "1"
    assert may[ai] == "0"

    # the March log was never given flags -> blank cells, not zeros
    march = by_key[
        "s3://ics-1ahs-prod/data/TPG/2026.03/collection_c/22.45.01_checkout_run/"
    ]
    assert march[di] == ""
    assert march[ai] == ""
